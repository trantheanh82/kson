...
class MY_Controller extends MX_Controller {
	...
	**/
	...
	function insert_assets($files,$template,$type,$admin = false){
		if($type=='css'){
			return insert_css($files,$template,$admin);
		}

		if($type = 'js'){
			return insert_js($files,$template,$admin);
		}
	}
...
function insert_css($files,$template='default',$admin = false){
	...
	$r = rand(0,190298712979);
	if($admin == true) $link .= 'admin/';

	$assets = "";
	if(is_array($files)){
		foreach($files as $file){
			$assets .= "<link href='".$link.($file[0]=='/'?'':'/css/').$file."?r=".$r."' rel='stylesheet' type='text/css' />\n";
		}
	}else{
		return "<link href='".$link.($files[0]=='/'?'':'/css/').$files."?r=".$r."' rel='stylesheet' type='text/css' />\n";
	}
	return $assets;
}

function insert_js($files,$template='default',$admin = false){
	$link = base_url().'assets/'.$template;
	...
	if(is_array($files)){
		foreach($files as $file){
			$assets .= "<script type='text/javascript' src='".$link.($file[0]=='/'?'':'/js/').$file."'></script>\n";
		}
	}else{
		return "<script type='text/javascript' src='".$link.($files[0]=='/'?'':'/js/').$files."'></script>\n";
	}
	return $assets;
}
